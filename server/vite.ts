...
import { type Server } from "http"; // Import Server type from "http"
...
export async function setupVite(app: Express, server: Server) {
  const vite = await createViteServer({
    ...
    configFile: false,
    customLogger: {
      ...viteLogger, // Spread the default logger to include all required methods
      error: (msg, options) => {
    ...
    } as Logger, // Type assertion to ensure it matches Logger interface
    server: {
      middlewareMode: true,
      hmr: { server },
    },
